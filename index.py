...
COLLECTION_ID = os.getenv("TRINAXAI_COLLECTION_ID", config.DEFAULT_COLLECTION_ID).strip() or config.DEFAULT_COLLECTION_ID
COLLECTION_NAME = os.getenv("TRINAXAI_COLLECTION_NAME", config.DEFAULT_COLLECTION_NAME).strip() or config.DEFAULT_COLLECTION_NAME
APPEND_ONLY = os.getenv("TRINAXAI_INDEX_APPEND", "0").strip().lower() in {"1", "true", "yes", "on"}
...
def collect_files(root: str) -> list[str]:
    ...
    for dirpath, dirnames, filenames in os.walk(root):
        ...
        for fn in filenames:
            if fn.startswith("."):
                continue
            if fn.lower() not in allowed_names and os.path.splitext(fn)[1].lower() not in allowed:
                continue
    ...
    if skipped_big:
        print(f"   ⏭️  {skipped_big} archivos omitidos por tamaño "
...
def build_nodes(documents: list[Document]) -> list:
    ...
    for doc in documents:
        ...
        if language:
            try:
                ...
                print(f"   ⚠️  AST falló en {os.path.basename(file_path)} "
    ...
    print(f"   └─ {code_count} por AST, {prose_count} por texto "
...
if __name__ == "__main__":
    ...
    if incremental:
        ...
        deleted = [] if APPEND_ONLY else [r for r in old_state if r.startswith(prefix) and r not in new_state]
        ...
        node_ids = [nid for nid, n in index.docstore.docs.items()
