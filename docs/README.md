...
This directory is the entry point for the technical and operational documentation of **TrinaxAI 1.2.6**, the current Production/Stable release under **AGPL-3.0-or-later**. It documents the release branch. For critical settings and endpoints, also verify `.env.example`, `chat-pwa/package.json`, and FastAPI's generated OpenAPI specification.

For the product overview and benchmarks, see the official website: **[trinaxai.app](https://www.trinaxai.app/)**. Repository screenshots of the daily loop are available here:

| Flow | Capture |
| --- | --- |
| Chat with citations | [PNG](assets/screenshots/chat-citations-en.png) |
| Indexing job | [PNG](assets/screenshots/indexing-job-en.png) |
| Pairing | [PNG](assets/screenshots/pairing-en.png) |
| Agent approval | [PNG](assets/screenshots/agent-approval-en.png) |

> Release status: `v1.2.6` is the current Production/Stable release. Its source archives, installers, wheel, checksums, and detached signatures are published on GitHub. Installers are pinned and never fall back to `main`.
...
Use the release-pinned commands below. They verify the installer SHA-256 before
execution on both Unix and Windows.
