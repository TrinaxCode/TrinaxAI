...
Este directorio es el punto de entrada a la documentación técnica y operativa de **TrinaxAI 1.2.6**, la release actual Production/Stable bajo **AGPL-3.0-or-later**. La documentación describe la rama de release; cuando una opción o endpoint sea crítico, confirma también su valor en `.env.example`, `chat-pwa/package.json` o en la especificación OpenAPI expuesta por FastAPI.

Para la vista general del producto y los benchmarks, consulta la web oficial: **[trinaxai.app](https://www.trinaxai.app/)**. Las capturas del repositorio sobre el loop diario están aquí:

| Flujo | Captura |
|---|---|
| Chat con citas | [PNG](assets/screenshots/chat-citations-es.png) |
| Trabajo de indexación | [PNG](assets/screenshots/indexing-job-es.png) |
| Pairing | [PNG](assets/screenshots/pairing-es.png) |
| Aprobación del agente | [PNG](assets/screenshots/agent-approval-es.png) |

> Estado del release: `v1.2.6` es la release actual Production/Stable. Sus paquetes fuente, instaladores, wheel, checksums y firmas separadas están publicados en GitHub. Los instaladores están fijados y nunca vuelven a `main`.
