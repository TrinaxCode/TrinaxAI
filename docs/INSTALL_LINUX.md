...
> Release status: `v1.2.0` is the current candidate, but its GitHub Release assets are not published yet. The command below is ready for the published release; for immediate testing, use the local checkout with `bash install.sh`. The installer intentionally refuses to fall back to `main`.
