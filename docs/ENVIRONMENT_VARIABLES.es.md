...
| `TRINAXAI_PROFILE` | `16gb` | Perfil de hardware. Los valores y alias aceptados viven en `trinaxai_core.VALID_PROFILES`. |
...
| `TRINAXAI_ALLOW_LAN_SYSTEM` | `0` | Activa el fallback heredado de LAN privada solo cuando no existe token admin. Prefiere pairing con scopes. |
| `TRINAXAI_ADMIN_TOKEN` | vacío | Supercredencial administrativa para operaciones remotas protegidas. Prefiere tokens de dispositivo con privilegio mínimo. |
...
| `TRINAXAI_EMBED_PRESET` | derivado del perfil | Preset de embeddings: `balanced`, `quality`, `max`, `lite` o `fast`. |
...
| `TRINAXAI_SEARXNG_URL` | vacío | URL base de una instancia SearXNG local o remota. |
...
| Variable | Predeterminado | Propósito |
|---|---|---|
| `TRINAXAI_INDEX_DIR` | padre del repositorio | Carpeta recorrida recursivamente por el indexador. |
...
| `TRINAXAI_INDEX_LOCK_TIMEOUT` | `3600` | Segundos de espera del lock del escritor del índice. |
...
| `TRINAXAI_RAG_MIN_SCORE` | `0.05` | Score mínimo aceptado en Knowledge mode explícito. |
...
| `VITE_TRINAXAI_INDEX_DIR` | vacío | Hint inicial de carpeta de índice mostrado por la PWA. |
...
requiere reindexar por completo. Nunca confirmes `.env`, tokens, certificados
locales, `storage/` ni `local_sources/`.
