<h1 align="center">
  <a href="https://www.trinaxai.app/"><img src="chat-pwa/public/logo.webp" alt="TrinaxAI" width="144" valign="middle"></a> · 🧪 Installer Testing
</h1>
...
> Release status: `v1.2.6` is Production/Stable. Its GitHub Release assets are published and the release-pinned installers never fall back to `main`.
...
When a Windows host is not available, the PowerShell dry-run suite can still
be exercised with the official PowerShell container:
...
This validates the current checkout's PowerShell parsing, dry-run behavior, and
paths containing spaces. It does not emulate Windows APIs or Windows
PowerShell 5.1; the authoritative Windows check is the `windows-2025` job in
...
`.github/workflows/test-installers.yml` runs syntax and dry-run checks on the pinned `ubuntu-24.04`, `macos-15`, and `windows-2025` runners, including a Windows PowerShell 5.1 pass. It does not install Ollama or attempt to emulate another operating system. The release workflow publishes source archives and URL installers and verifies every published download URL.
