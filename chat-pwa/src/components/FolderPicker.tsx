...
export default function FolderPicker({ initialPath, onSelect, onClose }: FolderPickerProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className={`relative z-10 flex h-[70vh] max-h-[560px] w-full max-w-lg flex-col overflow-hidden rounded-2xl border shadow-2xl ${closing ? 'animate-modal-out' : 'animate-modal-in'} ${panel}`}>
        {/* Header */}
        <div className={`flex shrink-0 items-center gap-2 border-b px-4 py-3 ${isDark ? 'border-white/10' : 'border-gray-200'}`}>
          ...
          <button onClick={handleClose} className={`ml-auto rounded-lg p-1.5 ${rowHover}`} aria-label={t('close')}>
            <MdClose size={18} />
          </button>
        </div>
